...
export default function ReportsPage() {
  ...
  const loadReports = useCallback(
    async (usePage = 1) => {
      try {
        ...
        if (res.pagination && typeof res.pagination.totalItems === "number") {
          ...
          setTotalPages(
            res.pagination.totalPages ||
              Math.max(
                1,
                Math.ceil(res.pagination.totalItems / (period.limit || limit)),
              ),
          );
        }
      } catch (err) {
        ...
        showNotification(
          "error",
          "Error al cargar reportes: " + (err.message || err),
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleMassiveDownload = async (fmt) => {
    try {
      setDownloading(true);
      ...
      const {
        ...
        startDate: sd,
        endDate: ed,
      } = await ReportService.getMassiveReport({
        startDate: format(startDate, "yyyy-MM-dd"),
        endDate: format(endDate, "yyyy-MM-dd"),
        origin: origin && origin !== "todos" ? origin : undefined,
        destination:
      ...
      // Construir filas — mismo formato que ReportModal
      const rows = [
        [
          "Código Servicio",
          "Origen",
      ...
      const fileName = `reporte_${sd}_${ed}`;
      ...
      showNotification(
        "success",
        `Reporte de ${details.length} servicios descargado`,
      );
    } catch (err) {
      showNotification("error", "Error al generar descarga: " + err.message);
    } finally {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-col items-end gap-2">
            <div className="flex gap-2">
              <Button
                variant="outline"
                onClick={() => handleMassiveDownload("csv")}
              ...
              <Button
                variant="outline"
                onClick={() => handleMassiveDownload("xlsx")}
                disabled={downloading || loading}
                className="gap-2"
              >
                {downloading ? (
            ...
            {downloading && (
              <p className="text-sm text-muted-foreground">
                Esta descarga podría demorar varios minutos...
              </p>
            )}
          </div>
        </div>
                    ...
                    {services.map((s, idx) => (
                      ...
                      >
                        ...
                        <TableCell>
                          <Badge
                            variant={
                              s.totalPassengers > 0 ? "default" : "outline"
                            }
                            className={
                              s.totalPassengers > 0
                        ...
                        <TableCell>
                          <Badge
                            variant={
                              s.availableSeats > 0 ? "default" : "outline"
                            }
                            className={
                              s.availableSeats > 0
                        ...
                        <TableCell className="text-right">
                          <Button
                            ...
                            size="icon"
                            onClick={() =>
                              openServiceDetail(s.serviceId || s._id)
                            }
                            className="h-8 w-8"
                          >
